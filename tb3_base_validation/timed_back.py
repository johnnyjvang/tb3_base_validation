...
Move backward for a fixed amount of time, then stop.
...
class TimedBack(Node):
    def __init__(self):
        super().__init__('timed_back')
    ...
    def loop(self):
        ...
        if self.done:
            if time.time() - self.finish_time > 0.5:
                self.get_logger().info('Exiting timed_back')
                rclpy.shutdown()
            return
        ...
        if self.start_time is None:
            ...
            self.get_logger().info('Starting timed backward motion')

        elapsed = time.time() - self.start_time

        # Move backward until the timer expires
